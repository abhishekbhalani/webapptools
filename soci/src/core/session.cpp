...
#ifdef SOCI_USE_LOG4CXX
void session::set_log4cxx_logger(log4cxx::LoggerPtr s){
	log4cxx_logger = s;
}
#endif
...
#ifdef SOCI_USE_LOG4CXX
		LOG4CXX_TRACE(log4cxx_logger, query);
#endif
        lastQuery_ = query;
    }
}
...
int session::get_affected_rows(statement & stmt)
{
	return backEnd_->get_affected_rows(*stmt.get_backend());
}
