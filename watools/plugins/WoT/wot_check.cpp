...
{
    LOG4CXX_TRACE(logger, _T("wot_check::get_interface ") << ifName);
    if (iequals(ifName, "wot_check")) {
        LOG4CXX_DEBUG(logger, _T("wot_check::get_interface found!"));
        usageCount++;
        return (this);
    }
...
void wot_check::process( webEngine::task* tsk, webEngine::scan_data_ptr scData )
{
    ...
    if (sit == req_list.end() || sit->second == false) {
        ...
        LOG4CXX_TRACE(logger, _T("wot_check::process - search data for ") << domain);
...
void wot_check::process_response( webEngine::i_response_ptr resp )
{
    ...
    LOG4CXX_DEBUG(logger, _T("wot_check::process_response: ") << rurl);

    domain = resp->RealUrl().host;
    if (ht_resp->HttpCode() > 399) {
        LOG4CXX_WARN(logger, _T("wot_check::process_response - bad respose: ") << ht_resp->HttpCode() << _T(" ") << rurl);
        req_list[domain] = false;
    } // if error
    else if (ht_resp->HttpCode() > 299) {
        LOG4CXX_WARN(logger, _T("wot_check::process_response - want redirect"));
        req_list[domain] = true; // prevent sending requests
        string url = ht_resp->Headers().find_first("Location");
        if (!url.empty()) {
            LOG4CXX_DEBUG(logger, _T("wot_check::process_response redirected to ") << url);
            bool to_process = true;
            transport_url baseUrl = ht_resp->BaseUrl();
            baseUrl.assign_with_referer(url);
            LOG4CXX_DEBUG(logger, _T("wot_check::process_response: reconstructed url is ") << baseUrl.tostring());
            result = i_request_ptr(new webEngine::HttpRequest(url));
            result->depth(0);
            result->ID(pluginInfo.plugin_id);
            result->context = this;
