...
void signal_halt(int sig)
{
    ...
    if (sig == 0) {
        ...
        LOG4CXX_INFO(scan_logger, "Signal received - exit reporter");
    }
}
...
void send_keepalive(webEngine::i_storage* store, int timeout) {
    ...
    webEngine::db_cursor rec = packet.push_back();
    rec["modules.id"] = reporter_instance;
...
void send_plugins_list(int timeout) {
    ...
    string db_key = "ScanModule:PlugIns:" + scaner_uuid + ":" + boost::lexical_cast<string>(reporter_instance);
    webEngine::plugin_list plgs = we_dispatcer->get_plugin_list();
    for (int i = 0; i < plgs.size(); i++)
    {
...
string get_command(webEngine::i_storage* store)
{
    ...
    c_instance.operation() = webEngine::db_condition::equal;
    c_instance.value() = scaner_uuid + ":" +  boost::lexical_cast<string>(reporter_instance);
...
int dispatcher_routine(po::variables_map& vm)
{
    ...
    storage->get(scan_query, modules);
    ...
    LOG4CXX_TRACE(scan_logger, "Found " << reporter_instance << " instances of " << scaner_uuid);
    reporter_instance++;
    int max_inst = vm["instances"].as<int>();
    if (reporter_instance > max_inst) {
        LOG4CXX_FATAL(scan_logger, "Can't run instance " << reporter_instance << " 'cause the limit is " << max_inst);
        return retcode;
    }
    
    // init values 
    sys_cpu();
    queue_key = "ScanModule:Queue:" + scaner_uuid + ":" + boost::lexical_cast<string>(reporter_instance);
    ...
    LOG4CXX_DEBUG(scan_logger, "Register instance #" << reporter_instance);
