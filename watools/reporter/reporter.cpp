...
#ifdef WIN32
string default_config = ".\\reporter.conf";
wstring default_trace  = L".\\watools.reporter.log";
#else
string default_config = "/etc/watools/reporter.conf";
wstring default_trace  = L"/tmp/watools.reporter.trace";
#endif
    ...
    if (val == "yes") {
        ...
        try {
            ...
            if (noLogging) {
                ...
                LOG4CXX_WARN(module_logger, _T("Can't parse '") << val << _T("' as bool: ") << string(e.what()) << _T(". Assume false."));
            }
        }
        if (i != 0) {
            retval = true;
...
void save_config(const string& fname, po::variables_map& vm, po::options_description& desc, bool noLogging = false )
{
    ...
    try {
            ...
            if (vm.count(name)) {
                ...
                if (is_any<string>(val)) {
                    ...
                    if (noLogging) {
                        ...
                        LOG4CXX_ERROR(module_logger, _T("Unknown variable type ") << string(val.type().name()));
                    }
                }
            } else {
        ...
        if (noLogging) {
            ...
            LOG4CXX_ERROR(module_logger, _T("Configuration not saved: ") << string(e.what()));
        }
        return;
    }
    if (!noLogging) {
...
restart:
    ...
    LOG4CXX_INFO(module_logger, _T("WAT Reporter started. Version: ") << reporter_version);
    ...
    if (vm.count("identity")) {
        ...
        LOG4CXX_INFO(module_logger, _T("WAT Reporter identificator is undefined, create new"));
        basic_random_generator<boost::mt19937> gen;
        uuid tag = gen();
        module_uuid = boost::lexical_cast<string>(tag);
        // todo: save config
    ...
    LOG4CXX_INFO(module_logger, _T("WAT Reporter identificator is ") << module_uuid);
    ...
    LOG4CXX_TRACE(module_logger, _T("running as ") << string(argv[0]));
...
finish:
    ...
    LOG4CXX_INFO(module_logger, _T("WAT Reporter stopped"));
    return 0;
}
