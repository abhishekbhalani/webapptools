...
#define MODULE_CLASS_CONSOLIDATOR    0
...
void signal_halt(int sig)
{
    ...
    if (sig == 0) {
        ...
        LOG4CXX_INFO(scan_logger, "Signal received - exit consolidator");
    }
}
...
void send_keepalive(webEngine::i_storage* store, int timeout) {
    ...
    rec["modules.id"] = module_instance;
    rec["modules.instance"] = module_uuid;
    rec["modules.class"] = MODULE_CLASS_CONSOLIDATOR;
    ...
    c_class.value() = MODULE_CLASS_CONSOLIDATOR;
    c_id.field() = "modules.id";
    c_id.operation() = webEngine::db_condition::equal;
    c_id.value() = module_instance;
...
int dispatcher_routine(po::variables_map& vm)
{
    ...
    LOG4CXX_TRACE(scan_logger, "Found " << module_instance << " instances of " << module_uuid);
    module_instance++;
    int max_inst = vm["instances"].as<int>();
    if (module_instance > max_inst) {
        LOG4CXX_FATAL(scan_logger, "Can't run instance " << module_instance << " 'cause the limit is " << max_inst);
        return retcode;
    }
    ...
    LOG4CXX_DEBUG(scan_logger, "Register instance #" << module_instance);
