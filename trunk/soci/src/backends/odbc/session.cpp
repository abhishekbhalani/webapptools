...
int odbc_session_backend::get_affected_rows(details::statement_backend & stmt){
	SQLINTEGER rowcount = 0;
	try{
		SQLRETURN rc = SQLRowCount( dynamic_cast<odbc_statement_backend*>(&stmt)->hstmt_, &rowcount);
		if (is_odbc_error(rc))
		{
			throw odbc_soci_error(SQL_HANDLE_DBC, hdbc_, "getting row count");
		}
	}catch(std::bad_cast e){
		throw odbc_soci_error(SQL_HANDLE_DBC, hdbc_, "Wrong statement!");
	}
	return rowcount;
}
